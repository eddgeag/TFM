...
plotPCA_methods <- function (X.list,factor, title1,title2, size = 1.5, glineas = 0.25,labels.scores,labels.loads) {
  ...
  colores<-1:length(levels(Group))
   ...
   p1 <- ggplot(X.scores,aes(x=PC1, y=PC2)) +
     ...
     geom_vline(xintercept = 0, color = "gray70") +
     geom_point(aes(color = Group,shape=Group2), alpha = 0.55, size = 3) +
     coord_cartesian(xlim = c(min(X.scores[,1])-0.05,max(X.scores[,1]))) +
     scale_fill_discrete(name = "Group")
   # avoiding labels superposition
...
spca.perf <- function(X,fix_parameter,transpose){
  ...
  if(transpose==F){
    ...
    for( i in 1:length(parameter)){
       perf.vector[i]<-perf(as.matrix((X)),t(spca((X),alpha=alpha,beta=beta[i],verbose = F,k=ncol(X))$loadings))
  
    }
    beta <- beta[which.min(perf.vector)]
    pcx<- spca(X,alpha=alpha,beta=beta,verbose=F,k=ncol(X))
    
  }else if(transpose==T){
    
        for( i in 1:length(parameter)){
       perf.vector[i]<-perf(as.matrix(t(X)),(spca(t(X),alpha=alpha,beta=beta[i],verbose = F,k=ncol(X))$scores))
  
    }
    beta <- beta[which.min(perf.vector)]
    pcx<- spca(t(X),alpha=alpha,beta=beta,verbose=F,k=ncol(X))
  }
    ...
    for( i in 1:length(parameter)){
       perf.vector[i]<-perf(as.matrix((X)),t(spca((X),alpha=alpha[i],beta=beta,verbose = F,k=ncol(X))$loadings))
    ...
    pcx<- spca(X,alpha=alpha,beta=beta,verbose=F,k=ncol(X))
        ...
        for( i in 1:length(parameter)){
       perf.vector[i]<-perf(as.matrix(t(X)),(spca(t(X),alpha=alpha[i],beta=beta,verbose = F,k=ncol(X))$scores))
