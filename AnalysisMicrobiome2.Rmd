...
res_iso_phylum <-plotMDS(nmds_ids_phylum,grupos,title = "Phylum Jaccard-similarity",labels = rownames(X.phylum),method="iso")
res_iso_phylum$grafico1
```
...
# 
res_meta_phylum <-plotMDS(nmds_meta_phylum_soersen,grupos,title = "Phylum Soersen-similarity",labels = rownames(X.phylum),method="meta")

res_meta_phylum$grafico1
...
```{r}
plot_phylum<-plotCA_methods(ca.phylum,grupos,"phylum",labels.bac = colnames(X.phylum),labels.samples = rownames(X.phylum),size=3)

grid.arrange(plot_phylum$plot_cols,
plot_phylum$plot_rows)
```
...
for(i in 1:length(variables)){
  if(variables[i]=="Order"||variables[i]=="Sample"){
    next
  }else if(class(phylum.rel[,i])=="factor"){
    next
  }
  variable <- variables[i] ## name of the variable to plot
  titulo <- colnames(phylum.rel)[i] # title of the varialbe
  
  breaks <- pretty(range(phylum.rel[,variable]), 
                   n = nclass.Sturges(phylum.rel[,variable]), min.n = 1)# make pretty histogram by appropiate bins
  bwidth <- breaks[2]-breaks[1] ## bin width

  ## plot
  hist.list.meta[[i]]<-ggplot(phylum.rel,aes_string(x=variable))+geom_histogram(aes(y=..density..),
...
dim(X.genus)
X.genus<-X.genus[,grep("^ud-",invert=T,colnames(X.genus))]
dim(X.genus)
```
...
mds.genus.plot<-plotPcoA(mds.genus,grupos,"Genus Jaccard similarity",labels = rownames(X.genus))
...
```{r}
iso.genus.plot <-plotMDS(nmds_ids_genus,factor = grupos,title = "Soersen Index Genus",method = "iso",labels=rownames(X.genus))
meta.genus.plot <-plotMDS(nmds_meta_genus_soersen,factor = grupos,title = "Soersen Index Genus",method = "meta",labels=rownames(X.genus))
grid.arrange(iso.genus.plot$grafico1,meta.genus.plot$grafico1)
```
