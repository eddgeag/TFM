...
```{r}
common.bacteria <- function(datos_i,datos){

  ## retorna los indices con las variables comunes

  interaccion <- interaction(datos$GROUP,datos$OBESE)
  ...
  for(i in 1:6){
   w[[names(w)[i]]]<-which(apply(as.matrix(datos_i[
    rownames(datos_i)%in%
      datos[datos$interaccion==grupo[i],"Paciente"],]),2,function(x) sum(x)==0))
  }
...
```{r}
low.count.removal_counts = function(
                        data, # OTU count data frame of size n (sample) x p (OTU)
...
result.filter.genus <- low.count.removal_counts(genus, abundance=10)
...
genus.clr <- logratio.transfo(as.matrix(genus.filter), logratio = 'CLR', offset = 1)
genus.abs <- genus.filter
...
sse.microbiome<- SummarizedExperiment(assays = t(v$E),
                                         colData=columnData)
contrasts(obesidad)<-contr.sum(n=length(levels(obesidad)))
contrasts(grupo)<-contr.sum(n=length(levels(grupo)))
design <- model.matrix(~1+obesidad*grupo)
rownames(design)<-variables_in_bacteria$Paciente
nf <-calcNormFactors(genus.abs,lib.size = colSums(genus.abs))
voom.norm <- voom(t(genus.abs),lib.size = nf,design = design,plot = T)

csa <-data.frame(t(voom.norm$E))
csa$grupo <-variables_in_bacteria$Paciente
csa.melt <- melt(csa,variable.name = "grupo")
colnames(csa.melt) <- c("grupo","genero","value")
ggplot(csa.melt,aes(value,color=grupo))+geom_density()
```
...
plsda.log <- plsda(datos,Y=interaccion,scale = T)
...
annotate_figure(p5,bottom = "none")
p6<-ggarrange(p3,p4,ncol = 2,common.legend = T)
annotate_figure(p6,bottom = "none")
...
revisar documentacion de DESEQ2, agrupar el factor y hacer contrastes
...
sse.microbiome<- SummarizedExperiment(assays = (voom.norm$E),
                                         colData=columnData)
contrasts(obesidad)<-contr.sum(n=length(levels(obesidad)))
contrasts(grupo)<-contr.sum(n=length(levels(grupo)))

design <- model.matrix(~1+obesidad*grupo)
fit.microbiome <- lmFit(assay(sse.microbiome),design = design,method = "robust",weights = v$weights) #ajusta un modelo lineal por cada variable mirar residuos normales y homocedasticidad y outliers
fit.microbiome.ebayes <- eBayes(fit.microbiome)
(toptablemio <- topTable(fit.microbiome.ebayes,coef=2:6,number = 37,adjust.method = "fdr",confint = T))
resultados <- decideTests(fit.microbiome.ebayes)@.Data  ## mira exactamente lo que queremos
resultados
...
<!-- fit.metaboloma <- lmFit(assay(sse.metaboloma),design = design,method = "robust") #ajusta un modelo lineal por cada variable mirar residuos normales y homocedasticidad y outliers -->
<!-- fit.metaboloma.ebayes <- eBayes(fit.metaboloma) -->
<!-- toptablemio <- topTable(fit.metaboloma.ebayes,coef=2:6,number = 37,adjust.method = "BH",confint = T) -->
<!-- resultados <- decideTests(fit.metaboloma.ebayes)@.Data  ## mira exactamente lo que queremos -->
