...
```{r}
nombres.metabolitos <- colnames(datos_integrar.metaboloma)
nombres.generos <- colnames(datos_integrar.microbioma)
mapeado <- Reduce(rbind,lapply(nombres.generos, function(x) cbind(x,nombres.metabolitos)),)
colnames(mapeado)<-c("genero","metaboloma")
...
```{r}
## nos sirve para ver si hay o no hay asociacion en una variable
executePermutation <- function(n){replicate(n,sample(x,replace = T))}


p.partial_met <- apply((datos_integrar.metaboloma), 2, function(x) kruskal.test(x,grupo)$p.value)

p.partial_mic <- apply((datos_integrar.microbioma), 2, function(x) kruskal.test(x,grupo)$p.value)
```
