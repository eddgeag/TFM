...
result.filter.genus <- low.count.removal(genus.offset, percent=0.1)
...
result.filter.genus.rel <- low.count.removal(genus.rel, percent=0.01)

genus.filter.rel <- result.filter.genus.rel$data.filter
```
...
contrasts(grupo)<-contr.sum(n=length(levels(grupo)))
design <- model.matrix(~obesidad*grupo)
fit.microbiome <- lmFit(assay(sse.microbiome),design = design,method = "robust") #ajusta un modelo lineal por cada variable mirar residuos normales y homocedasticidad y outliers
fit.microbiome.ebayes <- eBayes(fit.microbiome)
(toptablemio <- topTable(fit.microbiome.ebayes,coef=5:6,number = 37,adjust.method = "BH",confint = T))
