...
sse.microbiome<- SummarizedExperiment(assays = t(genus.clr),
                                         colData=columnData)
...
fit.microbiome <- lmFit(assay(sse.microbiome),design = design,method = "ls") #ajusta un modelo lineal por cada variable mirar residuos normales y homocedasticidad y outliers
fit.microbiome.ebayes <- eBayes(fit.microbiome)
(toptablemio <- topTable(fit.microbiome.ebayes,coef=3:4,number = 37,adjust.method = "fdr",confint = T))
