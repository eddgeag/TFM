...
```{r}
library(STATegRa)
exprs<- datos_integrar.metaboloma
pData <- columnData
phenoData<-AnnotatedDataFrame(pData)
sseMet <- ExpressionSet(assayData = t(exprs),phenoData = phenoData)

exprs<- datos_integrar.microbioma
pData <- columnData
phenoData<-AnnotatedDataFrame(pData)
sseMic <- ExpressionSet(assayData = t(exprs),phenoData = phenoData)


```

```{r}
csa <- omicsNPC(list(metabolome=sseMet,microbiome=sseMic),numCores = 4,outcomeName = "interaccion")
