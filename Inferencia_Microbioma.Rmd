...
```{r}
csa <-prcomp(as.matrix(euclidean),center=T)
varianzas <- 100*(csa$sdev^2/sum(csa$sdev^2))
graphPCA.methods(csa$x,datos.common = datos.common,varianzas = varianzas)
```


```{r}
csa2 <-cmdscale(euclidean)
graphPCA.methods(csa2,datos.common = datos.common,varianzas = varianzas)
...
y <- datos[,"Neisseria"]
summary(aovp(y~obesos*grupo))
pairwise.wilcox.test(y,g=interaccion,p.adjust.method = "BH")
```
