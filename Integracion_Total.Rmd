...
datos.microbioma <- (as.matrix(genus.clr))
datos.clinic <- t((Clin))
datos.IP <- t((IP.basal))
datos <- list(metaboloma=(datos.metaboloma),
IP=datos.IP,Clinic=datos.clinic,microbioma=datos.microbioma)
...
ggstatsplot::ggbetweenstats(datos.gg[datos.gg$factor=="Factor2",],y=value,x=interaccion,type="np")
```
...
outerprod <-t((as.matrix((weights.recast[,2:5])) %*% t(as.matrix(factores.recast[,2:5]))))
