...
### Primera opcion Bray Curtis
```{r}
genus.rel <- genus.abs/rowSums(genus.abs) # primero hacemos a proporciones
sorensen <- vegdist(genus.rel)
mds <- cmdscale(sorensen,k=2,eig=T)
varianzas <- 100*mds$eig/sum(mds$eig)
(res <- adonis2(sorensen~obesidad*grupo))
graphPCA.methods(mds$points,datos.common = datos.common,varianzas)
```
...
### Segunda opcion
```{r}
genus.clr <- as.data.frame((matrix(logratio.transfo(t(genus.rel),offset = 2,logratio="ILR"),nrow=94,ncol=46)))
colnames(genus.clr) <- rownames(genus.abs)
rownames(genus.clr) <- colnames(genus.abs)
euc <- vegdist(t((genus.clr)),method = "euclidean")
mds <- cmdscale(euc,k=2,eig=T)
varianzas <- 100*mds$eig/sum(mds$eig)
(res <- adonis2(euc~obesidad*grupo))
graphPCA.methods(mds$points,datos.common = datos.common,varianzas)

p1<-fn.densidad(((t(genus.clr))),interaccion)
p1
p2 <- fn.boxplot(t(genus.clr),interaccion)
p2
```
### Tercera opcion
...
genus.imp <- cmultRepl(genus.abs)
genus.clr <- as.data.frame((matrix(logratio.transfo(t(genus.imp),offset = 2,logratio="ILR"),nrow=94,ncol=46)))
colnames(genus.clr) <- rownames(genus.abs)
rownames(genus.clr) <- colnames(genus.abs)
euc <- vegdist(((genus.imp)),method = "aitchison")
mds <- cmdscale(euc,k=2,eig=T)
varianzas <- 100*mds$eig/sum(mds$eig)
(res <- adonis2(euc~obesidad*grupo))
graphPCA.methods(mds$points,datos.common = datos.common,varianzas)
...
```{r}
(res <- adonis2(euc~obesidad*grupo,add = T,sqrt.dist = T))
```
...
(res<-adonis2(euc~obesidad*grupo,method = "euclidean"))
...
```{r}
library(MOFA2)
datos.metaboloma<-((voom(met37_norm,normalize.method = "quantile")$E))
datos.microbioma <- matrix(logratio.transfo(t(genus.imp),logratio = "ILR",offset = 0.5),nrow=94,ncol=46)
colnames(datos.microbioma) <- rownames(genus.imp)
rownames(datos.microbioma) <- colnames(genus.imp)
datos.clinic <- t((Clin))
datos.IP <- t((IP.basal))
datos <- list(metaboloma=datos.metaboloma,IP=datos.IP,Clinic=datos.clinic,microbioma=datos.microbioma)
MOFAobject <-create_mofa_from_matrix(datos)
samples_metadata(MOFAobject)<- data.frame(sample=pacientes,obesidad=obesidad,grupo=grupo)
...
model_opts$likelihoods=c("gaussian","gaussian","gaussian","gaussian")
...
# lapply(data_mdl_mofa, function(x) lapply(x, dim))[[1]]
# ```
...
outerprod <-  as.matrix(factores.recast[,2:4]) %*% t(weights.recast[,2:4])
...
mds <- as.data.frame(cmdscale(dist(outerprod)))
mds$obesidad <- factores.recast$obesidad
mds$grupo <- factores.recast$grupo
...
permanova <- adonis2(outerprod[,-c(154,153,152)]~outerprod$obesidad+outerprod$grupo +outerprod$obesidad:outerprod$grupo)
permanova
```
