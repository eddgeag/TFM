...
ggarrange(plot_top_weights(model,scale=T,view=1),plot_weights(model,scale = T,nfeatures = 10))
plot_data_scatter(model,factor=1,view=1,features = top_weights$features,color_by = "obesidad",shape_by = "grupo")


```
```{r}

p1 <- plot_factor(model,color_by  = "obesidad",group_by = "grupo",add_boxplot = T,dodge=T,add_violin = T,rasterize = F,factors = 1,scale = T)
p2 <- plot_factor(model,color_by  = "obesidad",group_by = "grupo",add_boxplot = T,dodge=T,add_violin = T,rasterize = F,factors = c(2,4),scale=T)
...
ggarrange(plot_top_weights(model,scale=T,view=2,factors = 4),plot_weights(model,scale = T,nfeatures = 10))
plot_data_scatter(model,factor=4,view=2,features = top_weights$features,color_by = "grupo",shape_by = "obesidad")

plot_data_scatter(model,factor=4,view=2,features = top_weights$features,color_by = "obesidad",shape_by = "grupo")
```


### pruebas de mantel sobre pesos
```{r}
factors <- get_factors(model, as.data.frame = T,scale = F,groups = "all",factors ="all")
...
top_weights.metagenoma <- as.character(plot_top_weights(model,scale = T,view =2,factors = 4)$data$feature)
top_weights.metaboloma <- as.character(plot_top_weights(model,scale = T,view =1,factors = 1)$data$feature)
...
```{r}
datos_in <- rbind(scale(model@data$metaboloma$group1),scale(model@data$microbioma$group1))
aux <- fn.explore(t(datos_in)[,pesos])
library(ape)
combinaciones <- t(combn(names(aux)[8:13],2))
p<-matrix(NA,nrow=1000,ncol=15)
for(it in 1:1000){
...
mantel.df$asociacion <- ifelse(mantel.df$p<0.05,"<0.05",NA)
mantel.df$no_asociacion <-  ifelse(mantel.df$p>0.05,">0.05",NA)
write.csv(mantel.df,"./mantel_grupo.csv")
...
aux <- fn.explore(t(datos_in)[,pesos])
library(ape)
combinaciones <- t(combn(names(aux)[1:2],2))
p<-vector("numeric",length=dim(combinaciones)[1])
...
aux <- fn.explore(t(datos_in)[,pesos])
library(ape)
combinaciones <- t(combn(names(aux)[6:7],2))
p<-vector("numeric",length=dim(combinaciones)[1])
