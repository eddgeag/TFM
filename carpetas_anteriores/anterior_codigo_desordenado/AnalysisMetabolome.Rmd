...
graficos <- plotPCA_methods(list.pcxt,factor = grupos,title1=" PCA classical scores ",title2 ="loadings",labels.scores = rownames(X),labels.loads=colnames(X),size=3)
...
```{r}
n<-nrow(X)
pcx.nipals <- chemometrics::nipals(scale(X),a=ncol(X),it = 200)
...
```{r}
graficos <- plotPCA_methods(list.nipals_1,factor = grupos,title1="scores ",title2 ="loadings",labels.scores = rownames(X),labels.loads=colnames(X),size=3)
p1 <- graficos[[1]] +ggtitle("Scores")
p2<- graficos[[2]]+ggtitle("Loadings")
