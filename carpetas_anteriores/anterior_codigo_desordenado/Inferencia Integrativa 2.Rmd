...
genus.imp <- cmultRepl(genus.rel)
genus.clr <- as.data.frame((matrix(logratio.transfo(t(genus.imp),offset = 0.5,logratio="CLR"),nrow=94,ncol=46)))
...
datos.clinic <- t((Clin))
datos.IP <- t((IP.basal))
datos <- list(metaboloma=datos.metaboloma,microbioma=datos.microbioma)
...
permanova <- adonis2(outerprod[,-c(134,133,132)]~outerprod$obesidad+outerprod$grupo +outerprod$obesidad:outerprod$grupo,method="euclidean")
permanova
```
...
```{r}
network(as.matrix(outerprod[,-c(134,133,132)]),cutoff =0.99)
```
