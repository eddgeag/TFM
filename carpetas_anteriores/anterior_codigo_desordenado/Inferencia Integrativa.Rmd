...
samples_metadata(MOFAobject)<- data.frame(sample=pacientes,obesidad=obesos,grupo=grupo,interaccion=interaccion)
...
ggstatsplot::ggbetweenstats(datos.gg[datos.gg$factor=="Factor3",],y=value,x=interaccion,type="parametric")
```
...
```{r}
factor3 <- factores.recast[,c(1,4,8,9,10)]
weight.3 <- weights.recast[,c(1,4,8)]
```



```{r}
names(weight.3$Factor3) <- weight.3$feature
names(factor3$Factor3) <- factor3$sample
outerprod <-t((weight.3$Factor3) %*% t(factor3$Factor3))
...
permanova <- adonis2(outerprod[,38:131]~outerprod$obesidad+outerprod$grupo +outerprod$obesidad:outerprod$grupo)
permanova
```
...
```{r}
(fit.ob <-hotelling.test((Obese),(No.Obese),perm = T,var.equal = T,shrinkage = T,B=100))
```
...
```{r}
weight.3[weight.3$bloque=="metabolome","Factor3"]<-scale(weight.3[weight.3$bloque=="metabolome","Factor3"])
boxplot((as.numeric(weight.3$Factor3))~weight.3$bloque)

wilcox.test(weight.3$Factor3~weight.3$bloque)
```
```{r}
plot(factor3$Factor3)
```

```{r}
plot(as.numeric(weight.3$Factor3))
```


```{r}
(asociacion <-hotelling.test(t(outerprod[,1:37]),t(outerprod[,38:131]),shrinkage = T,var.equal = F,perm = T,B=100))
```
