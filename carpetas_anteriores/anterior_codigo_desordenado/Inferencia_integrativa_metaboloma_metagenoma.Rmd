...
dim(metaboloma)
dim(microbioma)
X <- rbind(metaboloma,microbioma)
```


```{r}
met <- paste(rownames(metaboloma),"metaboloma",sep="_")

mic <- paste(rownames(microbioma),"microbioma",sep="_")

nombres <- c(met,mic)
cosa <- t(combn(nombres,2))
w1 <-which(apply(cosa,1,function(X) all(grepl("metaboloma",X))))
w2 <-which(apply(cosa,1,function(X) all(grepl("microbioma",X))))
w3 <- sort(c(w1,w2))
cosa[w3]<-NA
cosa <- cosa[complete.cases(cosa),]
```



```{r}
plot_factors(model,factors = c(1,5),color_by= obesidad,shape_by = "grupo",scale = T)
```


```{r}
plot(factores.recast[,2],col=grupo)
```
