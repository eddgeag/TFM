...
samples_metadata(MOFAobject)<- data.frame(obesidad=as.numeric(obesidad),grupo=as.numeric(grupo),edad=Clin$EDAD,BMI=Clin$BMI,WC=Clin$WC,WHR=Clin$WHR,SHGB=Clin$SHBG,hsCRP=Clin$hsCRP,TE2=Clin$Total_ESTR,E2=Clin$Free_ESTRA,sample=pacientes,interaccion=interaccion)
...
MOFAobject.trained@covariates<-data.frame(obesidad=as.numeric(obesidad),grupo=as.numeric(grupo),edad=Clin$EDAD,BMI=Clin$BMI,WC=Clin$WC,WHR=Clin$WHR,SHGB=Clin$SHBG,hsCRP=Clin$hsCRP,TE2=Clin$Total_ESTR,E2=Clin$Free_ESTRA,interaccion=interaccion)
MOFAobject.trained
```
...
correlate_factors_with_covariates(MOFAobject.trained, 
  covariates =colnames(MOFAobject.trained@covariates), 
  plot="log_pval"
)
```
...
```{r}
par(mfrow=c(1,2))
plot(factores.recast[,1],col=interaccion)
plot(factores.recast[,2],col=interaccion)

```

## miramos los weights (primer factor)
```{r}
...
```{r}
pcx <- prcomp(Y,scale=F)
csa <-plsda(Y,grupo,scale = F)
```

```{r}
graphPCA.methods(csa$variates$X,datos.common = datos.common,varianzas=c(1,2))
```
