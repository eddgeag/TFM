...
```{r}
genero <- as.data.frame(t.input)
  colnames(genero) <- pacientes
    rownames(genero) <- colnames(genus.abs)
metaboloma <- as.data.frame(metaboloma)

datos <- list(met=metaboloma,genero=genero)
```


```{r}
x <- matrix(rnorm(46*37),ncol=37,byrow = T)
y <- matrix(rnorm(46*94),ncol=94,byrow = T)
z <- matrix(rnorm(46*13),ncol=13,byrow = T)
